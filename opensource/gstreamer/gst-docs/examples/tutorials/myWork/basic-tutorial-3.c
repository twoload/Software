...
int main(int argc, char* argv[])
{
    ...
    if(!data.pipeline || !data.source || !data.video_convert || !data.audio_convert || !data.video_sink || !data.audio_sink){
        g_printerr ("Not all elements could be created.\n");
        return -1;
    }

    gst_bin_add_many (GST_BIN (data.pipeline), data.source, data.video_convert, data.audio_convert, data.video_sink, data.audio_sink, NULL);
    if ( !gst_element_link (data.video_convert, data.video_sink) ){
        g_printerr ("Element could not be linked (video).\n");
        gst_object_unref (data.pipeline);
        return -1;
    }
    if ( !gst_element_link (data.audio_convert, data.audio_sink) ){
        g_printerr ("Element could not be linked (audio).\n");
        gst_object_unref (data.pipeline);
        return -1;
    }
...
static void pad_added_handler (GstElement *src, GstPad *new_pad, CustomData *data){
    ...
    if( g_str_has_prefix (new_pad_type, "audio/x-raw")){
        ...
        //goto exit;
        g_print("sohn>>> it is raw audio'%s'. \n", new_pad_type);
        if(gst_pad_is_linked (audio_sink_pad)){
            g_print ("We are already linked. Ignoring(audio_sink_pad).\n");
            goto exit;
        }
        ...
        g_print("sohn>>> it is raw video '%s'. \n", new_pad_type);
        if(gst_pad_is_linked (video_sink_pad)){
            g_print ("We are already linked. Ignoring(video_sink_pad).\n");
            goto exit;
        }
        ...
        g_print("sohn>>> Else condition \n");
        g_print("sohn>>> go to exit '%s'\n", new_pad_type);
        goto exit;
    }
